    ...
    public class TokenAuthController : tmssControllerBase
    {
        ...
        public async Task<RefreshTokenResult> RefreshToken(string refreshToken)
        {
            ...
            try
            {
                ...
                var refreshTokenNew = CreateRefreshToken(await CreateJwtClaims(principal.Identity as ClaimsIdentity, user, tokenType: TokenType.RefreshToken));


                return await Task.FromResult(new RefreshTokenResult(accessToken, GetEncryptedAccessToken(accessToken), (int)_configuration.AccessTokenExpiration.TotalSeconds, refreshTokenNew, (int)_configuration.RefreshTokenExpiration.TotalSeconds));
            }
        ...
        public async Task LogOut()
        {
            if (AbpSession.UserId != null)
            {
                ...
                await _cacheManager.GetCache(AppConsts.TokenValidityKey).RemoveAsync(tokenValidityKeyInClaims.Value);
