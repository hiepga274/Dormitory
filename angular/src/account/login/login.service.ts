...
export class LoginService {
    ...
    private login(accessToken: string, encryptedAccessToken: string, expireInSeconds: number, refreshToken: string, refreshTokenExpireInSeconds: number, rememberMe?: boolean, twoFactorRememberClientToken?: string, redirectUrl?: string): void {
        ...
        if (redirectUrl) {
            ...
            let isLogin = this._localStorageService.get("tmss/abpzerotemplate_local_storage/isLogin");
            if (isLogin) { //đã có
                this._localStorageService.clear("tmss/abpzerotemplate_local_storage/isLogin");
            }
            this._localStorageService.setItem("isLogin", 1);
